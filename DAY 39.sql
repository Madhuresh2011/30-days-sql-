...
SELECT 
    ap.category, 
    COUNT(rr.product_id) / (SELECT COUNT(*) FROM return_records) * 100 AS percentage_return
FROM 
    amazon_products AS ap
JOIN 
    return_records AS rr 
ON 
    ap.product_id = rr.product_id
GROUP BY 
    ap.category
ORDER BY 
    percentage_return DESC;

    
-- DATE :-  18-12-2024     MADHURESH RAJ
