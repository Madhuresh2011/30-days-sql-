...
-- Insert example data into customers37 table
...
WHERE EXTRACT(MONTH FROM spending_date) = 3   -- MONTH(spending_date)
GROUP BY 1
),
CTE2   -- april_spend
AS
(SELECT 
    ...
    SUM(spending_amount) as total_spend
FROM spending_records
WHERE EXTRACT(MONTH FROM spending_date) = 4  -- MONTH(spending_date)
GROUP BY 1)
